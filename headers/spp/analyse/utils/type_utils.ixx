...
namespace spp::analyse::utils::type_utils {
    SPP_EXP using GenericInferenceMap = ankerl::unordered_dense::map<
    ...
    SPP_EXP auto relaxed_symbolic_eq(
        asts::TypeAst const &lhs_type,
        asts::TypeAst const &rhs_type,
        scopes::Scope const *lhs_scope,
        scopes::Scope const *rhs_scope,
    ...
    SPP_EXP auto is_type_comptime_indexable(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_array(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_tuple(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_variant(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_boolean(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_void(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_generator(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_runtime_indexable(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_try(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_function(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> bool;

    SPP_EXP auto is_type_recursive(
        asts::ClassPrototypeAst const &type,
        scopes::ScopeManager const &sm)
        -> std::shared_ptr<asts::TypeAst>;

    SPP_EXP auto get_attr_types(
    ...
    SPP_EXP auto is_index_within_type_bound(
        std::size_t index,
        asts::TypeAst const &type,
        scopes::Scope const &sm)
        -> bool;

    SPP_EXP auto get_nth_type_of_indexable_type(
        std::size_t index,
        asts::TypeAst const &type,
        scopes::Scope const &sm)
        -> std::shared_ptr<asts::TypeAst>;

    SPP_EXP auto get_generator_and_yield_type(
        asts::TypeAst const &type,
        scopes::ScopeManager const &sm,
    ...
    SPP_EXP auto get_all_attrs(
        asts::TypeAst const &type,
        scopes::ScopeManager const *sm)
        -> std::vector<std::pair<asts::ClassAttributeAst*, scopes::Scope*>>;

    SPP_EXP auto create_generic_cls_scope(
    ...
    SPP_EXP auto create_generic_fun_scope(
    ...
    SPP_EXP auto create_generic_sup_scope(
    ...
    SPP_EXP auto create_generic_sym(
    ...
    SPP_EXP auto register_generic_syms(
        std::vector<std::shared_ptr<scopes::Symbol>> const &external_generic_syms,
    ...
    SPP_EXP auto get_type_part_symbol_with_error(
        scopes::Scope const &scope,
    ...
    SPP_EXP auto get_namespaced_scope_with_error(
        scopes::ScopeManager const &sm,
        asts::IdentifierAst const &ns)
        -> scopes::Scope*;

    SPP_EXP auto deduplicate_variant_inner_types(
        asts::TypeAst const &type,
        scopes::Scope const &scope)
        -> std::vector<std::shared_ptr<asts::TypeAst>>;

    SPP_EXP auto substitute_sup_scope_name(
        std::string const &old_sup_scope_name,
        asts::GenericArgumentGroupAst const &generic_args)
        -> std::string;

    SPP_EXP auto recursive_alias_search(
