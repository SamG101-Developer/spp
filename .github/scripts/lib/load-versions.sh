#!/usr/bin/env bash
# Publish the pinned toolchain and cache-key versions from
# .github/versions.env into the job environment, plus the
# install prefixes below. Lines in versions.env not of the
# form NAME=value are ignored. Note that this file is itself
# an input to the dependency cache key, because it is where
# SPP_LOCAL_PREFIX is chosen.
set -euo pipefail
...
# file cannot leave half its keys already exported.
while IFS= read -r line; do
  ...
  # Every pin is a version number, a git tag, a commit, a
  # digest or one release asset filename.
...

{
  ...
  # Install prefixes. Everything the project installs for
  # itself goes under $HOME: no step needs sudo, and the
  # tree can be handed straight to actions/cache. They are
  # not in versions.env because that file is copied into
  # GITHUB_ENV, where $HOME would not expand.
  #
