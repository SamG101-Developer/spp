...
class Task(object):
    ...
    Additionally we store the last execution time, so that next time the test is
    executed, the slowest tests are run first.
    """

    def __init__(
            self, test_binary, test_name, test_command, execution_number,
...
class TaskManager(object):
    ...
    def __init__(
            self, times, logger, test_results, task_factory, times_to_retry,
    ...
    def log_exit(self, task):
        with self.stdout_lock:
            ...
            if task.exit_code != 0:
                ...
                with open(task.log_file, encoding="utf-8", errors="replace") as f:
...
def execute_tasks(
        tasks, pool_size, task_manager, timeout_seconds,
