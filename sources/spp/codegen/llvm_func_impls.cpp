...
#define MAKE_BIN_OP_CLOSURE(Func) \
    [&ctx](llvm::Value *a, llvm::Value *b) { return ctx->Builder.Func(a, b, "result" + spp::utils::Uid()); }

#define MAKE_UN_OP_CLOSURE(Func) \
    [&ctx](llvm::Value *a) { return ctx->Builder.Func(a, "result" + spp::utils::Uid()); }

#define MAKE_CONV_CLOSURE(Func) \
    [&ctx, ty](llvm::Value *a) { return ctx->Builder.Func(a, ty, "result" + spp::utils::Uid()); }

auto spp::codegen::func_impls::std_abort_abort(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
...
auto spp::codegen::func_impls::std_boolean_bit_and(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
...
auto spp::codegen::func_impls::std_boolean_bit_ior(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateOr);
  simple_intrinsic_binop(sm, proto, ctx, llvm::Type::getInt1Ty(*ctx->Context), closure);
}

auto spp::codegen::func_impls::std_boolean_bit_xor(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateXor);
  simple_intrinsic_binop(sm, proto, ctx, llvm::Type::getInt1Ty(*ctx->Context), closure);
}

auto spp::codegen::func_impls::std_boolean_bit_not(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
...
auto spp::codegen::func_impls::std_boolean_and(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateLogicalAnd);
  simple_intrinsic_binop(sm, proto, ctx, llvm::Type::getInt1Ty(*ctx->Context), closure);
}

auto spp::codegen::func_impls::std_boolean_ior(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateLogicalOr);
  simple_intrinsic_binop(sm, proto, ctx, llvm::Type::getInt1Ty(*ctx->Context), closure);
}

auto spp::codegen::func_impls::std_boolean_eq(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateICmpEQ);
  simple_intrinsic_binop(sm, proto, ctx, llvm::Type::getInt1Ty(*ctx->Context), closure);
}

auto spp::codegen::func_impls::std_boolean_ne(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx) -> void {
...
auto spp::codegen::func_impls::std_memory_clear(
  ...
  const auto void_ty = llvm::Type::getVoidTy(*ctx->Context);
  const auto mem_ty = GetLlvmType(
    *sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::MemoryType(0, spp_ty).get()), ctx);
  const auto usize_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::USize(0).get()),
                                    ctx);

  const auto fn_ty = llvm::FunctionType::get(void_ty, {llvm::PointerType::get(*ctx->Context, 0)}, false);
  const auto fn = llvm::Function::Create(
    fn_ty, llvm::Function::ExternalLinkage, mangle::mangle_fun_name(*sm->CurrentScope, *proto), ctx->Module.get());
...
auto spp::codegen::func_impls::std_memory_place_element(
  ...
  const auto mem_ty = GetLlvmType(
    *sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::MemoryType(0, spp_ty).get()), ctx);
  const auto ptr_ty = GetLlvmType(
    *sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::SingleType(0, spp_ty).get()), ctx);
...
auto spp::codegen::func_impls::std_memory_take_element(
  ...
  // Define the types that will be used in the function.
  const auto ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(spp_ty.get()), ctx);
  const auto opt_ty = GetLlvmType(
    *sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::OptionType(0, spp_ty).get()), ctx);
  const auto mem_ty = GetLlvmType(
    *sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::MemoryType(0, spp_ty).get()), ctx);
  const auto ptr_ty = GetLlvmType(
    *sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::SingleType(0, spp_ty).get()), ctx);
  const auto size_ty = GetLlvmType(
    *sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::USize(0).get()), ctx);
  ...
  const auto some = ctx->Builder.CreateInsertValue(some_val, llvm::ConstantInt::getBool(*ctx->Context, true), {1},
                                                   "some");
  ...
  const auto none = ctx->Builder.CreateInsertValue(none_val, llvm::ConstantInt::getBool(*ctx->Context, false), {1},
                                                   "none");
  ctx->Builder.CreateBr(exit_bb);
...
//     const auto size_ty = sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::usize(0))->LlvmInfo->LlvmType;
//
...
auto spp::codegen::func_impls::std_intrinsics_add(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateAdd);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_add_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateAdd);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_sub(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateSub);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_sub_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateSub);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_mul(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateMul);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_mul_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateMul);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_sdiv(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateSDiv);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_sdiv_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateSDiv);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_udiv(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateUDiv);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_udiv_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateUDiv);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_srem(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateSRem);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_srem_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateSRem);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_urem(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateURem);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_urem_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateURem);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_sneg(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_UN_OP_CLOSURE(CreateNeg);
  simple_intrinsic_unop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_shl(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateShl);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_shl_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateShl);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_shr(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateLShr);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_shr_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateLShr);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_ior(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateOr);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_ior_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateOr);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_and(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateAnd);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_and_assign(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateAnd);
  simple_intrinsic_binop_assign(sm, proto, ctx, ty, closure);
}

auto spp::codegen::func_impls::std_intrinsics_bit_xor(analyse::scopes::ScopeManager const *sm,
  asts::FunctionPrototypeAst const *proto, LLvmCtx *ctx, llvm::Type *ty) -> void {
  const auto closure = MAKE_BIN_OP_CLOSURE(CreateXor);
  simple_intrinsic_binop(sm, proto, ctx, ty, closure);
}
