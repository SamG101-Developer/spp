...
auto spp::codegen::func_impls::std_memory_clear(
    ...
    const auto void_ty = llvm::Type::getVoidTy(*ctx->Context);
    const auto mem_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::MemoryType(0, spp_ty)), ctx);
    const auto usize_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::USize(0)), ctx);
...
auto spp::codegen::func_impls::std_memory_place_element(
    ...
    -> void {
    // Define the types that will be used in the function.
    const auto ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(spp_ty), ctx);
    const auto void_ty = llvm::Type::getVoidTy(*ctx->Context);
    const auto mem_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::MemoryType(0, spp_ty)), ctx);
    const auto ptr_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::SingleType(0, spp_ty)), ctx);
    const auto size_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::USize(0)), ctx);
...
auto spp::codegen::func_impls::std_memory_take_element(
    ...
    // Define the types that will be used in the function.
    const auto ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(spp_ty), ctx);
    const auto opt_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::OptionType(0, spp_ty)), ctx);
    const auto mem_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::MemoryType(0, spp_ty)), ctx);
    const auto ptr_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::SingleType(0, spp_ty)), ctx);
    const auto size_ty = GetLlvmType(*sm->CurrentScope->GetTypeSymbol(asts::generate::common_types::USize(0)), ctx);
