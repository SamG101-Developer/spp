...
auto spp::analyse::utils::func_utils::GetAllFunctionScopes(
  ...
  auto overload_scopes = Vec<std::tuple<
    scopes::Scope const*,
  ...
  if (target_scope->NsSym != nullptr) {
    for (auto *ancestor_scope : target_scope->Ancestors()) {
      for (auto const *sup_scope : ancestor_scope->Children | genex::views::ptr | genex::views::filter(
             is_valid_ext_scope)) {
  ...
  // Functions belonging to a type will have inheritance generics from "sup [...] Type { ... }"
    ...
    // cast is always valid because the function types are always the target - "$Func" etc.
    ...
    for (auto &&[scope_1, fn_1, _, _] : overload_scopes) {
      for (auto &&[scope_2, fn_2, _, _] : overload_scopes) {
        ...
        if (fn_1 != fn_2 and target_scope->DepthDiff(scope_1) < target_scope->DepthDiff(scope_2)) {
          ...
          if (conflict != nullptr) {
            overload_scopes |= genex::actions::remove_if([conflict](auto &&info) {
              return std::get<1>(info) == conflict;
            });
          }
...
auto spp::analyse::utils::func_utils::CheckForConflictingOverload(
  ...
  const auto existing_scopes = existing | genex::views::tuple_nth<0>;
  const auto existing_fns = existing | genex::views::tuple_nth<1>;
...
auto spp::analyse::utils::func_utils::NameGnArgs(
  ...
  NameGnArgsImpl<asts::GenericArgumentCompAst, asts::GenericParameterCompAst, asts::GenericParameterCompVariadicAst>(
    *comp_args, comp_params, owner, sm, meta);
  NameGnArgsImpl<asts::GenericArgumentTypeAst, asts::GenericParameterTypeAst, asts::GenericParameterTypeVariadicAst>(
...
auto spp::analyse::utils::func_utils::CreateCallablePrototype(
  ...
  auto dummy_param_group = MakeUnique<asts::FunctionParameterGroupAst>(nullptr, std::move(param_tys), nullptr);
