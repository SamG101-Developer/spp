...
auto spp::analyse::scopes::VariableSymbol::fq_name() const
    ...
    qualified_name = asts::ast_clone(std::get<ScopeIdentifierName>(scopes.back()->name).name->to<asts::IdentifierAst>());
    for (auto qualifier_scope : scopes | genex::views::reverse | genex::views::drop(1)) {
        const auto raw_ns_name = std::get<ScopeIdentifierName>(qualifier_scope->name).name->to<asts::IdentifierAst>();
