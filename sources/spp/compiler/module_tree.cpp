...
spp::compiler::ModuleTree::ModuleTree(
    ...
    for (auto &&m : vcs_modules) {
        ...
        if (inner_path != std::filesystem::path("src/main.spp") and not spp::utils::files::NativeString(inner_path).starts_with("ffi"s + std::filesystem::path::preferred_separator)) {
