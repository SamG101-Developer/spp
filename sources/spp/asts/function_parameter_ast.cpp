...
spp::asts::FunctionParameterAst::FunctionParameterAst(
    ...
    Type(std::move(type)) {
    SPP_SET_AST_TO_DEFAULT_IF_NULLPTR(this->TokColon, lex::SppTokenType::TK_COLON, ":", var != nullptr ? var->PosEnd() : 0);
    if (this->Var == nullptr) {
        const auto uid = spp::utils::Uid(this);
        const auto pos = this->Type != nullptr ? this->Type->PosStart() : 0uz;
        auto var_name = MakeUnique<IdentifierAst>(pos, uid);
...
auto spp::asts::FunctionParameterAst::Stage7_AnalyseSemantics(
    ...
    const auto ast = MakeUnique<LetStatementUninitializedAst>(nullptr, std::move(Var), nullptr, std::move(Type));
    ...
    for (const auto name : ExtractNames()) {
