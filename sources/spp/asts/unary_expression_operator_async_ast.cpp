...
auto spp::asts::UnaryExpressionOperatorAsyncAst::Stage11_CodeGen(
  ...
  // Generate the async closure function and set it in the future.
  {
    ...
    const auto fut_state_ptr_in_closure = ctx->Builder.CreateStructGEP(llvm_fut_type, fut_param, 0,
                                                                       "async.fut.state_ptr.in_closure" + uid);
    ...
    const auto fut_value_ptr_in_closure = ctx->Builder.CreateStructGEP(llvm_fut_type, fut_param, 1,
                                                                       "async.fut.value_ptr.in_closure" + uid);
