...
#include <../../../headers/spp/analyse/errors/semantic_error.hpp>
...
auto spp::asts::AnnotationAst::stage_2_gen_top_level_scopes(ScopeManager *sm, mixins::CompilerMetaData *meta) -> void {
    ...
    if (name->val == "virtual_method") {
        ...
        if (not fun_ctx) {
            errors::SppAnnotationInvalidApplicationError(*this, *m_ctx, "non-function").scopes({m_scope}).raise();
        }

        // The function ast must be a class method, not a free function.
        if (ctx_mod_ctx) {
            errors::SppAnnotationInvalidApplicationError(*this, *m_ctx, "non-class-method").scopes({m_scope}).raise();
        }

        // The "virtual_method" annotation cannot be applied to an abstract method.
        if (fun_ctx and fun_ctx->m_abstract_annotation) {
            errors::SppAnnotationConflictError(*this, *fun_ctx->m_abstract_annotation, *m_ctx).scopes({m_scope}).raise();
        }
        ...
        if (fun_ctx and fun_ctx->m_virtual_annotation and fun_ctx->m_virtual_annotation != this) {
            errors::SppAnnotationConflictError(*this, *fun_ctx->m_virtual_annotation, *m_ctx).scopes({m_scope}).raise();
        }
    }
        ...
        if (fun_ctx and fun_ctx->m_no_impl_annotation and fun_ctx->m_no_impl_annotation != this) {
            errors::SppAnnotationConflictError(*this, *fun_ctx->m_no_impl_annotation, *m_ctx).scopes({m_scope}).raise();
        }
    }
        ...
        if (not vis_ctx) {
            errors::SppAnnotationInvalidApplicationError(*this, *m_ctx, "non-visibility-enabled").scopes({m_scope}).raise();
        }

        // Access modifiers cannot be applied to methods in sup-ext blocks (only in module or sup).
        if (ctx_ext_ctx) {
            errors::SppAnnotationInvalidApplicationError(*this, *m_ctx->m_ctx, "extension").scopes({m_scope}).raise();
        }
        ...
        if (vis_ctx and vis_ctx->m_visibility.second and vis_ctx->m_visibility.second != this) {
            errors::SppAnnotationConflictError(*this, *vis_ctx->m_visibility.second, *m_ctx).scopes({m_scope}).raise();
        }
    }
        ...
        if (fun_ctx and fun_ctx->m_temperature_annotation) {
            errors::SppAnnotationConflictError(*this, *fun_ctx->m_temperature_annotation, *m_ctx).scopes({m_scope}).raise();
        }
    }
        ...
        if (fun_ctx and fun_ctx->m_inline_annotation) {
            errors::SppAnnotationConflictError(*this, *fun_ctx->m_inline_annotation, *m_ctx).scopes({m_scope}).raise();
        }
    }

    else {
        // Unknown annotations are not allowed.
        errors::SppAnnotationInvalidError(*this).scopes({m_scope}).raise();
    }
}
