    ...
    if (name->val == "virtual_method") {
        ...
        if (not fun_ctx) {
            analyse::errors::SemanticErrorBuilder<analyse::errors::SppAnnotationInvalidApplicationError>().with_args(
                *this, *m_ctx, "non-function").with_scopes({m_scope}).raise();
        }

        // The function ast must be a class method, not a free function.
        if (ctx_mod_ctx) {
            analyse::errors::SemanticErrorBuilder<analyse::errors::SppAnnotationInvalidApplicationError>().with_args(
                *this, *m_ctx, "non-class-method").with_scopes({m_scope}).raise();
        }
        ...
        if (not vis_ctx) {
            analyse::errors::SemanticErrorBuilder<analyse::errors::SppAnnotationInvalidApplicationError>().with_args(
                *this, *m_ctx, "non-visibility-enabled").with_scopes({m_scope}).raise();
        }
        ...
        analyse::errors::SemanticErrorBuilder<analyse::errors::SppAnnotationInvalidError>().with_args(
            *this).with_scopes({m_scope}).raise();
    }
}
